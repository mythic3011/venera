...
This document is the **full architecture reference / later-phase target** for Source Runtime Account + Request Policy.

Implementation execution Phase 1 is superseded by:
- `docs/plans/lane-c-phase1-minimal-runtime-core.md`

Use this file for long-horizon architecture direction, not as the immediate minimal implementation checklist.
...
- Keep legacy sources working through a best-effort runtime adapter for failure
  mapping and cutover only. This does not preserve old-core storage authority
  or legacy DB runtime contracts.
...
### Phase 1 - Minimal Runtime Core (Superseded Here)

Immediate implementation scope has been reduced and moved to:
- `docs/plans/lane-c-phase1-minimal-runtime-core.md`

This file no longer defines executable Phase 1 tasks.

### Later Phases - Full Architecture Rollout

- secure account store abstraction + source-scoped account index
- runtime request bridge
- `HeaderProfile` application in runtime pipeline
- cookie loading from profile snapshot
- `AccountValidatorHook`
- `ResponseClassifierHook` + strict fallback behavior
- migrate one reference source (preferably e-hentai/exhentai)
- `SessionRecoveryHook`
